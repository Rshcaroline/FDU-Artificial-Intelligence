...
class Node:
    def __init__(self, player=1, successor=[], isLeaf=False, value=None, action=None):
        ...
        self.isLeaf = isLeaf
        self.value = value
        # self.consider = is_considered # None: not available, True: should be considered, False: no need
...
def constructTree(n, board, player, action):
    """
    ...
    if prob_position is None:
        return None
    # print prob_pos
    # print board
    top_list = []
    # use brain_takeback() to trace back
    if n == 1:
        if len(prob_position) < expend_num:
            for pos in prob_position:
                ...
                x, y = pos[0], pos[1]
                brain_my(x, y)
                temp_value = board_evaluation(board)  # TODO: change
                successors.append(Node(player=3 - player, isLeaf=True, value=temp_value, action=pos))
                brain_takeback(x, y)
        else:
            for pos in prob_position:
        ...
        if len(prob_position) < expend_num:
            for pos in prob_position:
                ...
                x, y = pos[0], pos[1]
                brain_my(x, y)
                successors.append(constructTree(n - 1, board, 3 - player, pos))
                brain_takeback(x, y)
        else:
            for pos in prob_position:
                # board_copy = [[board[x][y] for y in range(pp.height)] for x in range(pp.width)]
                ...
                x,y = pos[0], pos[1]
                brain_my(x, y)
                top_list.append(board_evaluation(board))  # TODO: change
                brain_takeback(x, y)
            temp_list = top_list[:]
            temp_list.sort(reverse=True)
            for v in temp_list[0:expend_num]:
                ...
                # board_copy = [[board[x][y] for y in range(pp.height)] for x in range(pp.width)]
                # board_copy[pos[0]][pos[1]] = player
                # successors.append(constructTree(n - 1, board, 3 - player, pos, renew_probable_position(pos, prob_position)))
                pos = prob_position[top_list.index(v)]
                x, y = pos[0], pos[1]
                brain_my(x, y)
                successors.append(constructTree(n - 1, board, 3 - player, pos))
