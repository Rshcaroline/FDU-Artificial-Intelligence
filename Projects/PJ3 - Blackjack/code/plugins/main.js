...
function onLoad(assignmentId, ownerName, version) {
  ...
  header.append($('<div>')
    ...
    .append($('<div>').append('Addapted from <font color="red">Peeking Blackjack Stanford CS221</font>'.bold())));
  header.append($('<p>').append('TA in Charge: ' + ownerName));
//  header.append($('<p>').append('Version: ' + version));
  ...
  header.append($('<p>').append('This assignment has a written part and a programming part.'));
  var ol = $('<ol>').addClass('problem');
  if (assignmentId === 'foundations') { // TODO: remove this part
    ol.append($('<li>').addClass('writeup').append('This icon means a written answer is expected in <code>writeup.pdf</code>.'));
  } else {
    ol.append($('<li>').addClass('writeup').append('This icon means a written answer is expected in <code>' + assignmentId + '.pdf</code>.'));
  }
  ol.append($('<li>').addClass('code').append('This icon means you should write code in <code>submission.py</code>.'));
  header.append(ol);
  ...
  header.append($('<p>').append(
    ...
    'To run all the tests, type <pre>python grader.py</pre> ' +
  ...
  autoResult.parts.forEach(updatePoints);
  manualResult.parts.forEach(updatePoints);
  ...
  $('.writeup').each(showPoints);
  $('.code').each(showPoints);
